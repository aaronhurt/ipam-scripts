...
$formoptions = print_r($options, true);
...
{$formoptions}
  ...
  Optional:
    ...
    naptr_order=NUMBER        NAPTR Order
    naptr_pref=NUMBER         NAPTR Preference
    naptr_flag=STRING         NAPTR Flag(s)
    naptr_name=STRING         NAPTR Service name
    naptr_regex=STRING        NAPTR Regular expression
...
//         }
        ...
        list($status, $rows, $record) = ona_get_dns_record(array('dns_id' => $pointsto_record['id'], 'name' => $options['name'], 'domain_id' => $domain['id'],'type' => 'NS','dns_view_id' => $add_viewid));
        if ($rows or $status) {
    ...
    // Process NAPTR record types
    // NAPTR is a domain_id that points to another dns_id SRV record
    // this will give you "mydomain.com.           IN      NAPTR   2 0 "s" "CRAP+X2X" "" _crap._tcp.somedomain.com"
    else if ($options['type'] == 'NAPTR') {

        // Set interface id to zero
        // If there is no naptr_order set then stop (16 bit unsigned int)
        if (!isset($options['naptr_order']) or ($options['naptr_order'] < 0 or $options['naptr_order'] > 65536)) {
            printmsg("ERROR => You must provide an NAPTR order value between 0-65535 when creating NAPTR records!", 3);
            $self['error'] = "ERROR => You must provide an NAPTR order value between 0-65535 when creating NAPTR records!";
            return(array(4, $self['error'] . "\n"));
        }

        // If there is no naptr_pref set then stop (16 bit unsigned int)
        if (!isset($options['naptr_pref']) or ($options['naptr_pref'] < 0 or $options['naptr_pref'] > 65536)) {
            printmsg("ERROR => You must provide an NAPTR preference value between 0-65535 when creating NAPTR records!", 3);
            $self['error'] = "ERROR => You must provide an NAPTR preference value between 0-65535 when creating NAPTR records!";
            return(array(4, $self['error'] . "\n"));
        }

        // If there is no naptr_flag set then stop
        if (!isset($options['naptr_flag']) or !preg_match("/[A-Z0-9]/i", $options['naptr_flag'])) {
            printmsg("ERROR => You must provide an RFC compliant NAPTR flag value when creating NAPTR records!", 3);
            $self['error'] = "ERROR => You must provide an RFC compliant NAPTR flag value when creating NAPTR records!";
            return(array(4, $self['error'] . "\n"));
        }

        // If there is no naptr_name set then stop
        if (!isset($options['naptr_name']) or !preg_match("/[A-Z0-9]+\+[A-Z0-9]+/i", $options['naptr_name'])) {
            printmsg("ERROR => You must provide an RFC compliant NAPTR name value when creating NAPTR records!", 3);
            $self['error'] = "ERROR => You must provide an RFC compliant NAPTR name value when creating NAPTR records!";
            return(array(4, $self['error'] . "\n"));
        }

        // If there is no naptr_regex set then use empty value
        if (!isset($options['naptr_regex'])) {
            $options['naptr_regex'] = '';
        }
        ...
        // Find the domain name piece of $search
        list($status, $rows, $pdomain) = ona_find_domain($options['pointsto']);
        printmsg("DEBUG => ona_find_domain({$options['pointsto']}) returned: {$pdomain['fqdn']} for naptr pointsto.", 3);
        ...
        // Find the dns record that it will point to
        list($status, $rows, $pointsto_record) = ona_get_dns_record(array('name' => $phostname, 'domain_id' => $pdomain['id'], 'type' => 'SRV','dns_view_id' => $add_viewid));
        if ($status or !$rows) {
            printmsg("ERROR => Unable to find DNS SRV record to point SRV entry to!{$viewmsg}",3);
            $self['error'] = "ERROR => Unable to find DNS SRV record to point NAPTR entry to!{$viewmsg}";
            return(array(5, $self['error'] . "\n"));
        }

        // Validate that there are no records already with this domain and host
        list($status, $rows, $record) = ona_get_dns_record(array('dns_id' => $pointsto_record['id'], 'naptr_name' => $options['naptr_name'], 'domain_id' => $domain['id'], 'type' => 'NAPTR', 'dns_view_id' => $add_viewid));
        if ($rows or $status) {
            printmsg("ERROR => Another DNS NAPTR record for {$domain['fqdn']} with service name {$options['naptr_name']} pointing to {$options['pointsto']} already exists!{$viewmsg}",3);
            $self['error'] = "ERROR => Another DNS NAPTR record for {$domain['fqdn']} with service name {$options['naptr_name']} pointing to {$options['pointsto']} already exists!{$viewmsg}";
            return(array(5, $self['error'] . "\n"));
        }
        ...
        $add_interfaceid = 0;
        $add_dnsid = $pointsto_record['id'];
        $add_naptr_order = $options['naptr_order'];
        $add_naptr_pref = $options['naptr_pref'];
        $add_naptr_flag = $options['naptr_flag'];
        $add_naptr_name = $options['naptr_name'];
        $add_naptr_regex = $options['naptr_regex'];
    ...
    list($status, $rows) = db_insert_record(
        ...
        array(
            ...
            'naptr_order'          => $add_naptr_order,
            'naptr_pref'           => $add_naptr_pref,
            'naptr_flag'           => $add_naptr_flag,
            'naptr_name'           => $add_naptr_name,
            'naptr_regex'          => $add_naptr_regex,
    ...
    // TRIGGER: Update domain serial
    update_serial($add_domainid);
...
function dns_record_modify($options="") {
    ...
    if ($options['help'] or
       (!$options['set_name'] and
        ...
        !$options['set_naptr_order'] and
        !$options['set_naptr_pref'] and
        !$options['set_naptr_flag'] and
        !$options['set_naptr_name'] and
        !$options['set_naptr_regex'] and
...
Modify a DNS record
  ...
  Update:
    ...
    set_naptr_order=NUMBER      change NAPTR Order
    set_naptr_pref=NUMBER       change NAPTR Preference
    set_naptr_flag=STRING       change NAPTR Flag
    set_naptr_name=STRING       change NAPTR Name
    set_naptr_regex=STRING      change NAPTR RegEx
...
*/
    ...
    if ($options['set_name']) {
        ...
        if ($dns['type'] == 'CNAME' or $dns['type'] == 'MX' or $dns['type'] == 'NS' or $dns['type'] == 'SRV' or $dns['type'] == 'NAPTR') {
    ...
    if (array_key_exists('set_pointsto', $options) and ($options['set_type'] == 'CNAME' or $options['set_type'] == 'MX' or $options['set_type'] == 'NS' or $options['set_type'] == 'SRV' or $options['set_type'] == 'NAPTR')) {
        // Determine the host and domain name portions of the pointsto option
        // Find the domain name piece of $search
        ...
        if ($options['set_type'] == 'NAPTR') {
            list($status, $rows, $pointsto_record) = ona_get_dns_record(array('name' => $phostname, 'domain_id' => $pdomain['id'], 'type' => 'SRV','dns_view_id' => $check_dns_view_id));
            if ($status or !$rows) {
                printmsg("ERROR => Unable to find DNS SRV record to point NAPTR entry to!{$viewmsg}",3);
                $self['error'] = "ERROR => Unable to find DNS A record to point NAPTR entry to!{$viewmsg}";
                return(array(5, $self['error'] . "\n"));
            }
        }
    ...
    if (array_key_exists('set_naptr_order', $options) and $options['set_naptr_order'] != $dns['naptr_order'])
        $SET['naptr_order'] = $options['set_naptr_order'];
    if (array_key_exists('set_naptr_pref', $options) and $options['set_naptr_pref'] != $dns['naptr_pref'])
        $SET['naptr_pref'] = $options['set_naptr_pref'];
    if (array_key_exists('set_naptr_flag', $options) and $options['set_naptr_flag'] != $dns['naptr_flag'])
        $SET['naptr_flag'] = $options['set_naptr_flag'];
    if (array_key_exists('set_naptr_name', $options) and $options['set_naptr_name'] != $dns['naptr_name'])
        $SET['naptr_name'] = $options['set_naptr_name'];
    if (array_key_exists('set_naptr_regex', $options) and $options['set_naptr_regex'] != $dns['naptr_regex'])
        $SET['naptr_regex'] = $options['set_naptr_regex'];
    ...
    if(count($SET) > 0) {
        ...
        if ($changingint) {
            ...
            // TRIGGER: update domain serial
            update_serial($dnsrec['domain_id']);
        ...
        if($SET['domain_id']) {
            ...
            // TRIGGER: update the domain serial
            update_serial($SET['domain_id']);
        }
...
*/
	...
	// TRIGGER: update the domain serial
        update_serial($record['domain_id']);
