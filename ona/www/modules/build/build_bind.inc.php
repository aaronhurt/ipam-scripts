...
EOF
    ...
    // Start building the named.conf - save it in $text
    $text = "; DNS zone file for {$domain['name']} built on " . date($conf['date_format']) . "\n";
    ...
    while ($dnsrecord = $rs->FetchRow()) {
        ...
        if ($dnsrecord['type'] == 'NAPTR') {
            ...
            // Get the name info that the naptr points to
            list($status, $rows, $srv) = ona_get_dns_record(array('id' => $dnsrecord['dns_id']), '');
            ...
            $text .= sprintf("%-50s %-8s IN  %s %s %s %s %s %s %-30s %s\n" ,$name.'.',$dnsrecord['ttl'],$dnsrecord['type'],$dnsrecord['naptr_order'],$dnsrecord['naptr_pref'],
            '"'.$dnsrecord['naptr_flag'].'"','"'.$dnsrecord['naptr_name'].'"','"'.$dnsrecord['naptr_regex'].'"',$srv['fqdn'].'.',$dnsrecord['notes']);
        }
