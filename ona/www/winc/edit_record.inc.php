...
function ws_editor($window_name, $form='') {
    ...
    // If its a NAPTR, get the dns name for the SRV record it points to
    if ($dns_record['type'] == 'NAPTR') {
        list($status, $rows, $existingsrv_data) = ona_get_dns_record(array('id' => $dns_record['dns_id']));
        $dns_record['existingsrv_data'] = $existingsrv_data['fqdn'];
    }
    ...
    if ($dns_record['id']) {
        ...
        $dns_record['naptr_order'] = 0;
        $dns_record['naptr_pref'] = 0;
    ...
    array_push($record_types,'A','CNAME','MX','NS','SRV','TXT','PTR','NAPTR');
                ...
                <td class="padding" align="left" width="100%">
                    <select {$typedisable}
                        ...
                        onchange="var selectBox = el('record_type_select');
                                ...
                                el('naptr_container1').style.display   = (selectBox.value == 'NAPTR') ? '' : 'none';
                                el('naptr_container2').style.display   = (selectBox.value == 'NAPTR') ? '' : 'none';
                                el('txt_container').style.display   = (selectBox.value == 'TXT') ? '' : 'none';
                                el('name_container').style.display     = (selectBox.value == 'PTR') ? 'none' : '';
                                ...
                                el('existing_srv_container').style.display = (selectBox.value == 'NAPTR') ? '' : 'none';"
...
EOL;
    ...
    $window['html'] .= <<<EOL
            ...
            <!-- NAPTR CONTAINER -->
            <tr id="naptr_container1" style="display:none;">
                <td class="input_required" align="right" nowrap="true">
                    Order<br />Pref<br />Flag
                </td>
                <td class="padding" align="left" width="100%">
                    <input
                        style="margin-bottom:3px;"
                        id="set_naptr_order_{$window_name}"
                        name="set_naptr_order"
                        alt="NAPTR Order"
                        value="{$dns_record['naptr_order']}"
                        class="edit"
                        type="text"
                        size="5" maxlength="5"
                        onblur="updatednsinfo('{$window_name}');"
                    ...
                    <input
                        style="margin-bottom:3px;"
                        id="set_naptr_pref_{$window_name}"
                        name="set_naptr_pref"
                        alt="NAPTR Preference"
                        value="{$dns_record['naptr_pref']}"
                        class="edit"
                        type="text"
                        size="5" maxlength="5"
                        onblur="updatednsinfo('{$window_name}');"
                    /><br />
                    <input
                        id="set_naptr_flag_{$window_name}"
                        name="set_naptr_flag"
                        alt="NAPTR Flag"
                        value="{$dns_record['naptr_flag']}"
                        class="edit"
                        type="text"
                        size="5" maxlength="5"
                        onblur="updatednsinfo('{$window_name}');"
            ...
            <tr id="naptr_container2" style="display:none;">
                <td class="input_required" align="right" nowrap="true">
                    Name<br />RegEx
                </td>
                <td class="padding" align="left" width="100%">
                    <input
                        id="set_naptr_name_{$window_name}"
                        name="set_naptr_name"
                        alt="NAPTR Service Name"
                        value="{$dns_record['naptr_name']}"
                        class="edit"
                        type="text"
                        size="25" maxlength="64"
                        onblur="updatednsinfo('{$window_name}');"
                    /><br />
                    <input
                        id="set_naptr_regex_{$window_name}"
                        name="set_naptr_regex"
                        alt="NAPTR Regular Expression"
                        value="{$dns_record['naptr_regex']}"
                        class="edit"
                        type="text"
            ...
            <!-- EXISTING SRV CONTAINER -->
            <tr id="existing_srv_container" style="display:none;">
                <td class="input_required" align="right" nowrap="true">
                    Existing SRV record
                </td>
                <td class="padding" align="left" width="100%">
                    <input
                        id="set_srv_record_{$window_name}"
                        name="set_pointsto_srv"
                        alt="Points to existing SRV record"
                        value="{$dns_record['existingsrv_data']}"
                        class="edit"
                        type="text"
                        size="25" maxlength="64"
                        onblur="updatednsinfo('{$window_name}');"
                    />
                    <div id="suggest_set_srv_record_{$window_name}" class="suggest"></div>
                </td>
            </tr>

            <!-- EXISTING A CONTAINER -->
...
//     }
    ...
    // if this is an naptr ... reset form set_pointsto
    if ($form['set_type'] == 'NAPTR') $form['set_pointsto'] = $form['set_pointsto_srv'];
    ...
    if (!$form['dns_id']) {
        ...
        // if this is a cname, mx, ns, srv or naptr  set the pointsto option
        if ($form['type'] == 'CNAME' or $form['type'] == 'MX' or $form['type'] == 'NS' or $form['type'] == 'SRV' or $form['type'] == 'NAPTR') $form['pointsto'] = $form['set_pointsto'];
        ...
        if ($form['type'] == 'NAPTR') $form['naptr_order'] = $form['set_naptr_order'];
        if ($form['type'] == 'NAPTR') $form['naptr_pref'] = $form['set_naptr_pref'];
        if ($form['type'] == 'NAPTR') $form['naptr_flag'] = $form['set_naptr_flag'];
        if ($form['type'] == 'NAPTR') $form['naptr_name'] = $form['set_naptr_name'];
        if ($form['type'] == 'NAPTR') $form['naptr_regex'] = $form['set_naptr_regex'];
        ...
        // if this is not a cname or naptr clear pointsto.
        if ($form['set_type'] != 'CNAME' and $form['set_type'] != 'NAPTR') $form['set_pointsto'] == '';
    }


printmsg("DEBUG 2 => set_pointsto = {$form['set_pointsto']}", 3);
printmsg("DEBUG 2 => pointsto = {$form['pointsto']}", 3);
