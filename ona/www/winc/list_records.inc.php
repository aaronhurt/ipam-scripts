...
EOL;
        ...
        // Process SRV record
        if ($record['type'] == 'SRV') {
            // show the port value next to the type
            $record['type'] = "{$record['type']} ({$record['srv_pri']} {$record['srv_weight']} {$record['srv_port']})";
...
EOL;
        ...
        // Process NAPTR record
        if ($record['type'] == 'NAPTR') {
            // show the name and preference value next to the type
            $record['type'] = "{$record['type']} ({$record['naptr_order']} {$record['naptr_pref']} {$record['naptr_flag']} {$record['naptr_name']})";
            list($status, $rows, $naptr) = ona_get_dns_record(array('id' => $record['dns_id']), '');
            $data = <<<EOL
                    <a title="Edit DNS SRV record"
                       class="act"
                       onClick="xajax_window_submit('edit_record', 'dns_record_id=>{$record['dns_id']}', 'editor');"
                    >{$naptr['name']}</a>.<a title="View domain. ID: {$naptr['domain_id']}"
                         class="domain"
                         onClick="xajax_window_submit('work_space', 'xajax_window_submit(\'display_domain\', \'domain_id=>{$naptr['domain_id']}\', \'display\')');"
                    >{$naptr['domain_fqdn']}</a>.&nbsp;
EOL;
        }
